var store = [{
        ...
        "excerpt":"                                                                                                                                    한국어 메뉴얼                                                                                                      Learn More                                                                                                                                                           日本語マニュアル                                                                                                      Learn More                                                                                                                                                           中文手册                                                                                                      Learn More                                                                                                                                                           English Menual                                                                                                      Learn More                                       ","categories": [],
        "tags": [],
        ...
        "teaser":null},{
        "title": "1.ログイン",
        "excerpt":"  カクテルクラウドの初期ログイン画面は、以下のとおりである。   Role、ID、Passwordを入力後、ログイン。   （IDは管理者に別途確認必要）      最初の接続やパスワードの初期化時には、 必ずパスワードを変更する必要がある。      期間が90日を経過した場合にも、 パスワードを変更したり、 パスワードを延長することができる。      ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/docs/ja/0.6/",
        "teaser":null},{
        ...
        "teaser":null},{
        "title": "7.1.1 クラスター追加",
        "excerpt":"クラスターを追加登録することができる機能である。 a) クラスター → 右上の+ボタンをクリック。 b) 基本情報（クラスター名、k8sバージョン、説明）を入力。 基本情報 説明 クラスター名 登録するクラスタの名前 k8sバージョン クラスタにインストールされたKubernetesのバージョン情報。e.g）1.8.13 ID クラスタ固有のID（ユーザー指定）、アラームメッセージRedirect時に必要 説明 クラスタのユーザー説明 c) プロバイダ情報（アカウント、タイプ、地域）を入力。 アカウントのプロバイダとタイプに応じて入力欄が変更される。Baremetalの場合、地域はDefault値を提供し、変更が可能である。 プロバイダ 説明 アカウント 登録されたアカウント タイプ Kubernetesの使用タイプとして、MANAGED、PROVIER、GKEの中から選択 地域 Kubernetesがインストールされたサーバーのリージョン タイプ 説明 MANAGED CUBE Installerを利用して、kubernetesを構成したクラスタ PROVIDER Public CloudのVMでKubernetesを利用するが、 Public Cloudを利用する場合に使用する。 （ロードバランサやストレージなどその他のサービスを利用する場合） GKE Google Cloud PlatformのGoogle Kubernetes Engineで構成したクラスタ ㅤ プロバイダの種類 - MANAGED 追加入力...","categories": [],
        "tags": [],
        ...
        "teaser":null},{
        "title": "7.1.3 クラスター削除",
        "excerpt":"5.1.3 クラスター削除     クラスター情報を削除する機能である。 該当クラスターを使用しているアプリケーションマップがある場合、削除することができない。   a) クラスター → クラスター選択 → 登録 → 削除ボタンをクリック。     ","categories": [],
        "tags": [],
        ...
        "teaser":null},{
        "title": "7.5.1 アラームリスト",
        "excerpt":"アラームは、次のリストから発生条件が持続時間だけ継続した場合に発生する。 * #### AlertManager アラーム ID  ALM-001  レベル warning アラーム名 AlertmanagerDown 持続時間 5分 発生条件 Alertmanagerメトリックの収集ができない場合に発生 処理 Prometheusのログと Alertmanagerのログとイベントを確認する。 必要な場合には、Podを再起動する。 アラーム ID  ALM-002  レベル warning アラーム名 AlertmanagerFailedReload 持続時間 10分 発生条件 Alertmanagerの設定変更時、 設定の再読み込み処理失敗時に発生 処理 そのPodのログを確認して ConfigMapの設定エラーを修正する。 * #### ETCD3 アラーム ID  ETC-001  レベル  critical  アラーム名 InsufficientMembers 持続時間 3分 発生条件 ETCDメトリックの収集ができない場合に発生 処理 ETCDクラスタの状態を確認する。 Prometheusのログと、そのノードの etcd状態を確認する。...","categories": [],
        "tags": [],
        ...
        "teaser":null},{
        "title": "4.1.3 워크로드",
        "excerpt":" 어플리케이션을 구성하는 최소 단위이다. 1개의 워크로드가 어플리케이션이 될 수도 있고, 여러개의 워크로드가 하나의 어플리케이션을 구성 할 수 있다.   a) 좌측 메인메뉴 → 서비스 → 서비스 맵 선택하면 맵안에 구성된 워크로드들을 볼 수 있다.    ","categories": [],
        "tags": [],

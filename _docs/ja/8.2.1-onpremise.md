...
**1. インストールのために空のディレクトリを作成し、そのディレクトリに移動する。**
...
### Provider 別 cube.toml の書き方

#### Onpremise cube.toml の作成

| <U>[cube]</U> |
| 項目 | 説明 | 例 |
| ---- | ---- | ---- |
| version | kubernetes バージョン別 cube version | version = "1.13.5-r5" |
...
| cluster-type | クラスタの使用規模とHA構成するかどうかに応じて、タイプを区分 **[large/medium/Small/tiny/kaas]** [Learn more](../8.1.2.9) | cluster-type = "small" |
...
| api-sans | SAN(Subject Alternative Name) 認証書使用時、下記一覧作成 | api-sans = [] |
...
※ **etcd** は key-value storage で、すべての object(pod, rc, service, secret など)을 분산 저장한다.  
api server で間接的に Read/Write されるが、この時、楽観的なロックにより同時実行をコントロールし、プライオリティは [RAFT アルゴリズム](https://raft.github.io/) を活用する。  
この etcd インスタンスは奇数にしなければならない。  
最適なクラスターサイズについてはの詳細は [CoreOS ドキュメントを参考にする](https://coreos.com/etcd/docs/latest/v2/admin_guide.html#optimal-cluster-size).
...
| 項目 | 説明 | 例 |
| ---- | ---- | ---- |
| install | enterprise(cocktail-system) インストール状況をチェック | install = true |
...
#### 使用事例
* cube init コマンドで初期 cube.toml ファイルを生成し、Provider 情報及びインスタンス情報を記入する。
...
**※ ssh key で接続する**  
上記の項目で private_key_path と key\_path は、サーバーに ssh key で接続するための private key と public key のパスを入力する。  

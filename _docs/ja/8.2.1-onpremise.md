...
api server を介して間接的に読んで使われるこの時楽観的ロック方式を使用して同時実行制御をして、優先順位は、 [RAFT 알고리즘](https://raft.github.io/) を活用する。  
...
#### ユースケース
* cube init コマンドで初期 cube.toml 파을을 생성후 Provider 정보 및 인스턴스 정보를 기입한다.

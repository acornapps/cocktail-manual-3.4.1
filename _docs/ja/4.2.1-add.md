...
| ノードラベル値 | ノードラベル値 |
| アクセス権限（RBAC services Account） | アクセス権限RBAC値 |

* **サーバータイプがDaemonSetの場合に該当**
...
| 成功Job履歴制限 | 成功Job履歴制限（Successful Job History Limitations） |
| 失敗Job履歴制限 | 失敗Job履歴制限（Failed Job History Limitations） |
| 後続実行の一時停止 | Job履歴制限（Suspend） |

* **サーバータイプがJob/CronJobの場合に該当**
![]({{ site.baseurl }}/assets/EN/{{ site.version }}/4.2.1_26.png)

| **項目** | **説明** |
| :--- | :--- |
| Job完了回数 | Job完了回数（Completions）|
| 同時ロード | 同時ロード。0に設定したら値が増加する時までJobが一時中止される（Parallelism） |
| 活性期限（秒） | 活性期限（Active Deadline Seconds） |
| Back off Limit | backoffLimit |
| 終了後削除待機時間（秒） | TTL Seconds After Finished |
| 後続実行の一時停止 | Job履歴制限（Suspend） |

* **サーバータイプがStatefulの場合に該当**
...
* **Auto-Scale（サーバータイプがMultiの場合に該当）**
...
* **アップデートポリシー（サーバータイプがMulti/Stateful/Daemonの場合に該当）**
...
| パーティション | Statefulタイプの場合のみ選択。該当パーティションより大きい又は同一なバージョンはすべてのPodがアップデートされる。パーティションより低いバージョンのPodはアップデートせず、削除されても以前バージョンで再度作成される。 |

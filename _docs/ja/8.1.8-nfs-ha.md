...
Google Cloud 環境で Storage High Availibityのため、NFS serverを pacemaker、 corosync、 drbdを使って二重化する構成を説明します。
 - Corosync: クラスターのローレベルインフラを提供し、クラスタについてのコラム情報、メンバーシップ、安定的なメッセージングを提供
 - Pacemaker: クラスターリソース管理者。クラスターステータスによるリソース移動、ノード停止などを担当する。
...
* Load balancer 生成する。 (ex: 10.174.0.4 )
* Load balancer 設定する。
    - Backend configuration  : 1番で生成したインスタンスグループ選択後 2049ポート health check
...
* 以下の conf設定時 gcpは hostnameで変更 hostname または、IP変更
...
* NFS 2番で secondary を primary 設定
```bash
drbdadm primary r0
mount /dev/drbd0 /mnt
ls /mnt
...
* pacemaker 起動/中止
```bash
systemctl start pcsd    // NFS1, NFS2番に同時にスタート
systemctl cluster [start | stop] // 片っぽの装備で起動または終了
```
...
* GCE DISK 管理画面で NFSサーバに接続する 1,2 ディスクを同じサイズぐらい増やす。 (ex: 50G -> 200G)
* NFS サーバで物理的なディスクサイズが増えたのか確認
* DISK 論理適用
...
drbdadm status r0                           // ステータス照会
drbdsetup  status r0 --verbose --statistic  // ステータスの詳細照会
drbdsetup events2 --now r0                  // リアルタイムイベント照会
drbdadm cstate r0                           // ネットワークステータス照会
drbdadm role r0                             // リソース役割照会
drbdadm dstate r0                           // ディスクステータス照会
```

### 2. Pacemaker ステータスを照会
```bash
pcs status                                 // pacemaker clusterステータス照会
pcs cluster sync                           // リモートノードと構成同期化
pcs cluster reload corosync                // 構成情報 reload。 瞬断なし。
pcs alert config                           // Alert構成情報照会
pcs cluster corosync [node]                // corosync.conf構成情報照会
...
---
# Troubleshooting
### 1. NFS storage DRBD 接続切れてから両ノードが Primary役割になっている場合、手動で復旧方法
* DRBDが発生するalarm中、split brain発生を通知するalarmが発生できます。
* この場合は NFS serverのネットワーク接続が切れてから両ノードがPrimary役割ステータスを感知した場合、まず変更事項を廃棄するnode(victim)を選択し、次のコマンドで手動復旧します。
...
他のノード(survivorという)では、接続コマンドを実行するだけです。

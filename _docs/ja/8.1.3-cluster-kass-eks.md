...
参照: kubectl 設定 : <https://kubernetes.io/docs/tasks/tools/install-kubectl/>
  ...
  * 新しくインストールしたクラスタでは、初期 admin資格証明を取り出す必要はありません。
...
* **クラスタの状態がACTIVEになるまで待ちます。**   
※ クラスタが有効になる前に、Work nodeを起動すると、Work node がクラスタに登録されず、Work node を再起動する必要がある。
...
* Specify Details(詳細を指定) ページで、次のパラメータを入力し、選択する。
    ...
    * **ClusterName** :  
        Amazon EKS クラスタの作成時に使用する名前を入力します。  
        **重要** : この名前は、Amazon EKS クラスタ名と正確に一致しなければならない。一致しない場合 Work node を紐づけれらません。
...
**参照 :** <https://github.com/kubernetes-incubator/external-storage/tree/master/aws/efs>  
    ...
    * **kubectl apply -f 1-eks-cocktail-efs-rbac.yaml**  
    クラスタに RBAC が有効になっているか、OpenShiftを実行している場合、プロバイダに権限を付与する必要がある。  

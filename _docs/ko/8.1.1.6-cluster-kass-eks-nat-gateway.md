...
### 4. private work node 생성
    ...
    b. CloudFormation 화면으로 이동
        ...
        9. Worker Network Configuration 에서
            ...
            b. Subnets : Private subnets(위에서 생성한 2개 Private subnet-<A zone> , <B zone>) 만 선택 한다.

        10. create 한다 (5분소요 휴식)

        참고 : public IP 가 없기 때문에 Worker node에 접근 하기 위해서는 bastion 서버가 필요 하다.

### 5. 외부 SNAT(Source Network Address Translation) Disabled 설정

    a. **disable SNAT on your worker nodes**
    ...
    참조 : <https://docs.aws.amazon.com/ko_kr/eks/latest/userguide/external-snat.html>

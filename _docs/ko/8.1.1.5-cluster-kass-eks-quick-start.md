...
9. EFS(Elastic File System) 생성
    ...
    b. EFS 생성
        01. EFS > Create File System 버튼 클릭
        02. VPC : 4번에서 생성한 vpc 선택
        03. 하단의 Create mount targets의 security groups에 EFS 연동을 위해 생성한 security group만 선택
        04. Next Step 버튼 클릭
        05. Configure optional settings 화면에서 수정없이 "Next Step" 버튼클릭.
        06. Review and create 화면에서 "Create File System"버튼 클릭하여 생성함.
    
    c. EFS Provisioner Policy 생성 : Node에 EFS 권한 부여 하기
        01. IAM > Policies > Create policy
        02. JSON 탭 선택 : 아래내용 입력.
          ```
          {
              "Version": "2012-10-17",
              "Statement": [
          ...
          ```
          03. Review policy 클릭 > Create policy 클릭. policy를 생성 한다.
          04. IAM 에서 Worker 노드 인스턴스 역할에(예:eks-acloud-worker-NodeInstanceRole-4F2PSCQTAXJS) 위에서 만든  
            EFS Provisioner Policy 추가 한다.

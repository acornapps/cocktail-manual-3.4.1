...
* 인스턴스 화면에서 생성된 인스턴스(노드)의 **SSH**항목을 선택 -> **View gcloud command**를 선택 한다.

* **gcloud command line**을 복사 / 콘솔에서 실행 한다.

![gke-create-connect-node-ssh]({{ site.baseurl }}/assets/KR/{{ site.version }}/GKE/gke-create-connect-node-ssh.png)

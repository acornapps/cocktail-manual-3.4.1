...
---
#### [8.1.1.5 EKS 클러스터 만들기 Quick Start](../8.1.1.5)
#### [8.1.1.5 EKS 클러스터 만들기 NAT Gateway 사용](../8.1.1.6)
...
**관리자 권한 또는 해당 IAM 권한이 필요 한다.**
  ...
  * 만들어진 역할을 확인 한다.
  ...
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-role-1.png)
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-role-2.png)
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-role-3.png)
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-role-4.png) 
...
이 사용 설명서에서는 직접 생성을 선택 한다.  
  ...
  * VPC 만들기(Launch VPC Wizard)를 선택한다.
    ...
    * 1단계: VPC 구성 선택(Step 1: Select a VPC Configuration) :  
    ...
    ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-vpc-1.png)
  ...
  * Your VPCs 화면 목록에서 생성된 VPC를 확일 할 수 있다.
  ...
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-vpc-2.png)
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-vpc-3.png)  
...
클러스터를 만들 때 해당 클러스터를 위해 리소스를 호스팅할 모든 서브넷을 지정한다(예: 작업자 노드와 로드 밸런서).
  ...
  * VPC Service 의 서브넷(Subnets) 화면에서 확인할 수 있다.
  ...
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-subnet-1.png)
  ![eks-in-seoul]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-subnet-2.png)
...
* AWS VPC service 화면에서 보안 그룹을 선택한다.
    ...
    * Create를 선택한다.
    ...
    ![eks-create-security-group-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-security-group-1.png)
 -->
...
![eks-create-cloudformation-vpc-1-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cloudformation-vpc-1-1.png)
![eks-create-cloudformation-vpc-2-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cloudformation-vpc-2-1.png)
...
* AWS EKS 콘솔을 사용하여 클러스터를 생성 한다.
    ...
    * **5.** Status(상태) 필드는 클러스터 프로비저닝 프로세스가 완료될 때까지 CREATING(생성 중)으로 표시됩니다.  
        ...
        ![eks-create-cluster-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cluster-1.png)
        ![eks-create-cluster-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cluster-2.png)
        ![eks-create-cluster-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cluster-2-1.png)
        ![eks-create-cluster-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-cluster-2-2.png)
...
AWS IAM 권한이 있는 관리자에게 사용자 보안 자격 증명(액세스 키)을 요청 한다.
    ...
    * AWS 자격 증명 공급자 체인 만들기
        ...
        * 보안 자격 증명(Security credentials) 섹션 에서 액세스 키를 생성한다.
        ...
        ![eks-create-security-credentials]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-security-credentials-1.png)
...
* Specify Details(세부 정보 지정) 페이지에서 다음 파라미터를 입력하고 다음을 선택한다.
    ...
    * 생성된 노드 그룹에 대해 **NodeInstanceRole을** 기록한다. Amazon EKS Work node를 구성할 때 필요.
    ...
    ![eks-create-worker-node]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-worker-node.png)
    ![eks-create-worker-node]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/eks-create-worker-node-3.png)
...
* 목록에서 생성한 인스턴스 이름을 선택한 다음 작업을 선택한다.
    ...
    * 열린 메뉴에서 네트워킹과 보안 그룹 변경을 차례대로 선택한다.
    ...
    ![efs-create-security-group-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/efs-create-security-group-1.png)

    * 기본 VPC 보안 그룹이라는 설명이 있는 보안 그룹 옆 확인란을 선택한다.

    ![efs-create-security-group-2]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/efs-create-security-group-2-1.png)
    ...
    * 설명 탭에서 보안 그룹 – 옆에 두 항목이 나열되어 있는지 확인한다. 하나는 기본 VPC 보안 그룹용이고 다른 하나는 인스턴스 시작 시 생성했던 보안 그룹용입니다.
    ...
    ![efs-create-security-group-3]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/efs-create-security-group-3.png)
...
**참조 :** <https://github.com/kubernetes-incubator/external-storage/tree/master/aws/efs>  
    ...
    * 아래 명령으로 퍼시스턴트 볼륨을 확인 한다.
    ...
    ![efs-create-pv-1]({{ site.baseurl }}/assets/ZH/{{ site.version }}/EKS/efs-create-pv-1.png)  
